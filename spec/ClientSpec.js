...
const {UserAPI} = require('../dist/commonjs/user');
...
describe('Client', () => {
  ...
  describe('"init"', () => {
    it('loads an access token from the storage by default', (done) => {
      ...
      client.accessTokenStore.tokenStore.create(TABLE, PRIMARY_KEY, accessTokenData)
        .then((primaryKey) => {
  ...
  describe('"login"', () => {
    ...
    it('can login after a logout', (done) => {
      const client = new Client();
      client.login(loginData).then(() => client.logout()).then(done).catch(done.fail);
    });
  ...
  describe('"register"', () => {
    ...
    it('automatically gets an access token after registration', (done) => {
      const client = new Client();
      client.register(registerData).then((context) => {
